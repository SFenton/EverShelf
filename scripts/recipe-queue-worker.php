#!/usr/bin/env php
<?php
declare(strict_types=1);

if (PHP_SAPI !== 'cli') {
    http_response_code(403);
    exit('Forbidden');
}

define('CRON_MODE', true);
require_once __DIR__ . '/../api/bootstrap.php';

$options = [];
foreach (array_slice($argv, 1) as $argument) {
    if (!str_starts_with($argument, '--')) {
        continue;
    }
    $parts = explode('=', substr($argument, 2), 2);
    $options[$parts[0]] = $parts[1] ?? true;
}

$loop = isset($options['loop']);
$pollMilliseconds = max(
    50,
    min(
        30000,
        (int)($options['poll-ms'] ?? env(
            'RECIPE_QUEUE_LOCAL_POLL_MS',
            '1000'
        ))
    )
);
$limit = max(
    1,
    min(
        100,
        (int)($options['limit'] ?? env(
            'RECIPE_QUEUE_LOCAL_LIMIT',
            '20'
        ))
    )
);
$maxAttempts = max(
    1,
    min(
        20,
        (int)($options['max-attempts'] ?? env(
            'RECIPE_QUEUE_LOCAL_MAX_ATTEMPTS',
            '20'
        ))
    )
);
$databasePath = trim((string)($options['db'] ?? ''));
$socketPath = trim((string)(
    $options['socket'] ?? recipeJobWakeSocketPath()
));
$heartbeatPath = trim((string)($options['heartbeat'] ?? ''));
$statusPath = trim((string)($options['status-file'] ?? ''));
if (
    $socketPath === ''
    || strlen($socketPath) > 220
    || str_contains($socketPath, "\0")
) {
    throw new InvalidArgumentException(
        'recipe queue wake socket path is invalid'
    );
}
$directory = dirname($socketPath);
if (
    !is_dir($directory)
    && !mkdir($directory, 0775, true)
    && !is_dir($directory)
) {
    throw new RuntimeException(
        'recipe queue wake socket directory is unavailable'
    );
}

$writeState = static function (
    string $path,
    string $value
): void {
    if ($path === '') {
        return;
    }
    $temporary = $path . '.tmp.' . getmypid();
    if (
        file_put_contents($temporary, $value . PHP_EOL) === false
        || !rename($temporary, $path)
    ) {
        @unlink($temporary);
        throw new RuntimeException(
            'recipe queue worker state could not be written'
        );
    }
};

$workerLock = fopen(
    $directory . '/.recipe-queue-worker.lock',
    'c+'
);
if (
    $workerLock === false
    || !flock($workerLock, LOCK_EX | LOCK_NB)
) {
    if (is_resource($workerLock)) {
        fclose($workerLock);
    }
    echo json_encode([
        'success' => true,
        'skipped' => true,
        'reason' => 'recipe_queue_worker_already_running',
    ], JSON_UNESCAPED_SLASHES) . PHP_EOL;
    exit(0);
}

if (file_exists($socketPath) || is_link($socketPath)) {
    @unlink($socketPath);
}
$errno = 0;
$error = '';
$wakeSocket = @stream_socket_server(
    'udg://' . $socketPath,
    $errno,
    $error,
    STREAM_SERVER_BIND
);
if (is_resource($wakeSocket)) {
    stream_set_blocking($wakeSocket, false);
}

if ($databasePath === '') {
    $db = getDB();
} else {
    $databasePath = recipeCliAssertDatabaseInputSafe(
        $databasePath,
        isset($options['allow-active-db'])
    );
    $db = new PDO('sqlite:' . $databasePath);
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $db->setAttribute(
        PDO::ATTR_DEFAULT_FETCH_MODE,
        PDO::FETCH_ASSOC
    );
    $db->exec('PRAGMA foreign_keys=ON');
    $db->exec('PRAGMA busy_timeout=10000');
    databaseEnsureMigrated(
        $db,
        $databasePath . '.migration.lock'
    );
}

$running = true;
if (function_exists('pcntl_async_signals')) {
    pcntl_async_signals(true);
    pcntl_signal(SIGTERM, static function () use (&$running): void {
        $running = false;
    });
    pcntl_signal(SIGINT, static function () use (&$running): void {
        $running = false;
    });
}

$cycle = 0;
try {
    do {
        $cycle++;
        $writeState($heartbeatPath, (string)time());
        try {
            $result = recipeJobLocalWorkDue(
                $db,
                $maxAttempts
            )
                ? recipeJobProcessQueue(
                    $db,
                    $limit,
                    $maxAttempts,
                    false,
                    'local'
                )
                : [
                    'processed' => 0,
                    'succeeded' => 0,
                    'failed' => 0,
                    'skipped' => 0,
                    'items' => [],
                ];
            $writeState(
                $statusPath,
                '0 ' . time()
            );
            if (
                !$loop
                || (int)($result['processed'] ?? 0) > 0
                || (int)($result['failed'] ?? 0) > 0
            ) {
                echo json_encode([
                    'success' => true,
                    'cycle' => $cycle,
                    'lane' => 'local',
                    'poll_ms' => $pollMilliseconds,
                ] + $result, JSON_UNESCAPED_UNICODE
                    | JSON_UNESCAPED_SLASHES) . PHP_EOL;
            }
        } catch (Throwable $workerError) {
            databaseRollbackDanglingTransaction($db);
            $writeState(
                $statusPath,
                '1 ' . time()
            );
            EverLog::exception(
                $workerError,
                'recipe_queue_worker'
            );
            echo json_encode([
                'success' => false,
                'cycle' => $cycle,
                'lane' => 'local',
                'error' => mb_substr(
                    $workerError->getMessage(),
                    0,
                    500,
                    'UTF-8'
                ),
            ], JSON_UNESCAPED_UNICODE
                | JSON_UNESCAPED_SLASHES) . PHP_EOL;
            $result = [];
        }
        if (!$loop || !$running) {
            break;
        }
        if ((int)($result['processed'] ?? 0) > 0) {
            continue;
        }
        if (!is_resource($wakeSocket)) {
            usleep($pollMilliseconds * 1000);
            continue;
        }
        $read = [$wakeSocket];
        $write = null;
        $except = null;
        $seconds = intdiv($pollMilliseconds, 1000);
        $microseconds = ($pollMilliseconds % 1000) * 1000;
        $ready = @stream_select(
            $read,
            $write,
            $except,
            $seconds,
            $microseconds
        );
        if ($ready === false || $ready === 0) {
            continue;
        }
        while (@stream_socket_recvfrom($wakeSocket, 1024) !== false) {
        }
    } while ($running);
} finally {
    if (is_resource($wakeSocket)) {
        fclose($wakeSocket);
    }
    if (file_exists($socketPath) || is_link($socketPath)) {
        @unlink($socketPath);
    }
    flock($workerLock, LOCK_UN);
    fclose($workerLock);
}
